...
function filterSwaggerPathsByTagsOld(swagger, tagsToFilter) {
...
function includeSwaggerPath(pathDefinition, tagsToFilter) {
  if (!pathDefinition.operationId) {
    return true;
  }
  let tags = pathDefinition.tags;
  if (!tags) {
    tags = ['default'];
  }
  return tags.some(filterTagsF.bind(this, tagsToFilter.split(',')));
}

function filterSwaggerPathsByTags(swagger, tagsToFilter) {
  const newPaths = Object.keys(swagger.paths).reduce((finalPaths, path) => {
    const pathDef = swagger.paths[path];
    const filteredDef = Object.keys(pathDef).reduce((r, method) => {
      const methodDef = pathDef[method];
      if (includeSwaggerPath(methodDef, tagsToFilter)) {
        return Object.assign(r, { [method]: methodDef });
      }
      return r;
    }, {});
